...
from bot_loader import BotDefinitions, LadderZip
...
def main():
    ...
    files = [
        ("stockfish", None),
        (os.path.join("stockfish", "run.py"), "run.py"),
    ]
    definitions.add_bot(
        "stockfish", lambda params: Bot(Race.Terran, Stockfish()), LadderZip("stockfish", "Terran", files),
    )
    print(f"Available bots: {sorted(list(definitions.zippable.keys()))}")
    zip_types = definitions.zippable
    zip_keys = sorted(list(zip_types.keys()))
    zip_keys.append("all")
